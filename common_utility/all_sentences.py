...
def get_queries(file_path, doc_dir_path, required_qid):
...
def filter_document(doc_dir_path, start, end, single_dir, statistics):
    ...
    #print "need",statistics._doc_num
...
def select_sentence(documents, statistics):  
#print "document score for",did, "is", documents_score
    ...
    for doc in documents:
        ...
        for sid in doc["doc"]._sentences:
            ...
            single_sentence["time"] = doc["doc"]._time
...
def prepare_data(alpha, distribution, words, background, stopword_file):
...
def main():
    ...
    #parser.add_argument("sentence_mu")
    ...
    b = 1.0
    sentence_mu = 10000
    #para["output_file"] = "./result/%f-%f-%d" %(a,b,sentence_mu)
    run_id = "InfoTest" 
    ...
    queries = get_queries(para["query_file"], para["doc_dir_path"], args.required_qid)
    #result_dirs = 
    out_sentences = {}
    for qid in queries:
        ...
        out_sentences[qid] = []
        doc_result_list = []
        query_model, background_model, stopwords = prepare_data(para["alpha"], para["distribution"], queries[qid]._words, para["background"], para["stopwords"])
        ...
        for single_dir in queries[qid]._dirs:
            candidate_documents = filter_document(para["doc_dir_path"], queries[qid]._start, queries[qid]._end, single_dir, statistics)
            candidate_sentences = select_sentence(candidate_documents, statistics)
