...
def get_queries(file_path, doc_dir_path, requried_qid,stopwords):
    ...
    for event in root.iter("event"):
        ...
        for w in temp:
            ...
            if w not in words:
                words[w] = 1
        dirs =  [f for f in os.listdir(os.path.join(doc_dir_path,requried_qid)) if os.path.isfile(os.path.join(doc_dir_path,requried_qid,f))]
...
def get_documents_scores(doc_dir_path, start, end, single_dir, statistics, query_words,qid):
    ...
    for did in data:
        ...
        if time > int(start) and time < int(end):
            ...
            temp_doc = Document(sentences_dict,time,statistics, query_words)
...
def parse_args(para_file, data_dir):
    ...
    para["belta"] = float(root.find("belta").text)
    para["mu"] = float(root.find("mu").text)
    return para



def heap_build(result_list, documents,result_size, docids):
    result_size
    start_update =False
    processed = 0
    for did in documents:
        ...
        if m is None:
            ...
            if docid in docids:
                ...
                print "insert key", docid
                docids[docid] = 0

        if(start_update):
            item = heapq.heappushpop(result_list, (documents[did], did))
            m = re.match("\d+-(.+)",item[1])
            if m is None:
                print "wrong document id",did
                sys.exit(-1)
                ...
                try:
                    del docids[m.group(1)]
                except KeyError:
                    print "no such key",m.group(1)
                    sys.exit(-1)
        else:
            heapq.heappush(result_list, (documents[did], did))
            
            processed += 1
            if(processed >= result_size):
                start_update = True
...
def main():
    ...
    parser.add_argument("--data_dir","-d",default = "/home/1546/code/2015/new_required_data")
    parser.add_argument("qid")
    parser.add_argument("--stopword_file","-sf",default = "/home/1546/data/new_stopwords")
    ...
    parser.add_argument("--result_size",'-s',type=int, default = 20)

    args = parser.parse_args()
    #matches = get_matches(args.match_file)
    #rel_docs = matches["TS14.13"]
    args.data_dir = os.path.join(args.data_dir,args.qid)
    para = parse_args(args.para_file,args.data_dir)
    stopwords = read_stopwords(args.stopword_file)
    # print len(stopwords)
    # if "of" in stopwords:
    #     print "of in!"
    # raw_input("Press Enter....")
    para["output_dir"] = args.qid
    queries = get_queries(para["query_file"], para["doc_dir_path"], args.qid, stopwords)
    ...
    for qid in queries:
        ...
        for single_dir in queries[qid]._dirs:
            ...
            size = heap_build(doc_result_list, documents, args.result_size, docids)
