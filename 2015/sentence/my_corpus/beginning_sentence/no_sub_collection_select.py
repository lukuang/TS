...
def get_queries(file_path, doc_dir_path,required_qid,stopwords):
    ...
    for event in root.iter("event"):
        ...
        for w in temp:
            ...
            if w not in words:
                words[w] = 1
        dirs =  [f for f in os.listdir(doc_dir_path) if os.path.isfile(os.path.join(doc_dir_path,f))]
        #dirs = get_dir(doc_dir_path)
...
#print "document score for",did, "is", documents_score
    ...
    for doc in documents:
        ...
        size =  statistics._num_of_sentences
        index = 0
        candidate_sentences = []
        ...
        if len(all_sentences)==0:
            all_sentences = purified_sentences
        else:
            update_result(all_sentences,purified_sentences,statistics)
...
def prepare_data(alpha, beta, distribution, words, background, expansion_terms,stopwords):
...
def parse_args(para_file, required_qid):
    ...
    para["doc_dir_path"] = os.path.join(root.find("doc_dir_path").text,required_qid)
...
def purify_sentence(candidate_sentences, statistics):
    purified_sentences = []
    candidate_sentences.sort(key = lambda x: int(x["time"]))
...
def main():
    ...
    parser.add_argument("num_of_sentences",type=int)
    ...
    #run_id = "info_simple"
    para["output_file"] = "test-%d-%f-%d-%s" %(num_of_sentences, sim_threshold, sentence_mu,para["output_file"])
    #top_percent = 0.02
    #sim_threshold = 0.5
    doc_num = 10
    #doc_num = int(args.doc_num)
    expansion_terms = get_expansion_terms(args.term_dir)
    queries = get_queries(para["query_file"], para["doc_dir_path"], args.required_qid, stopwords)
    #result_dirs = 
    out_sentences = {}
    for qid in queries:
        ...
        for d in queries[qid]._dirs:
            print d
        query_model, background_model = prepare_data(para["alpha"], para["beta"],para["distribution"], queries[qid]._words, para["background"], expansion_terms[qid], stopwords)
        statistics = Statistics(query_model, background_model, para["mu"],\
            sentence_mu, num_of_sentences, sim_threshold, doc_num, stopwords)
