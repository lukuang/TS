...
def prepare_data(alpha, words, background, stopword_file, expansion_terms):
    ...
    for w in expansion_terms:
        query_model[w] = expansion_terms[w]*alpha
...
def main():
    ...
    for qid in queries:
        ...
        for d in queries[qid]._dirs:
            print d
        query_model, background_model, stopwords = prepare_data(para["alpha"],  queries[qid]._words, para["background"], para["stopwords"], expansion_terms[qid])
