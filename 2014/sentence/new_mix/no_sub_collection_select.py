...
def get_queries(file_path, required_qid, doc_dir_list):
    ...
    for doc in documents:
        ...
        for sid in doc["doc"]._sentences:
            ...
            for term in  single_sentence["model"]:
                if term in statistics._profile:
                    has_profile = True
                if term in statistics._wiki_model:
                    has_wiki = True
            if not (has_profile and has_wiki)):
                continue
...
def prepare_data(alpha, belta, distribution, words, background, stopword_file, expansion_terms):
    ...
    return query_model, background_model, stopwords, feedback_model, expansion_terms
...
def main():
    ...
    queries = get_queries(para["query_file"], args.required_qid, para["doc_dir_list"])
    #result_dirs = 
    out_sentences = {}
    for qid in queries:
        ...
        for d in queries[qid]._dirs:
            print d
        query_model, background_model, stopwords, profile, wiki_model = prepare_data(para["alpha"], para["belta"],para["distribution"], queries[qid]._words, para["background"], para["stopwords"], expansion_terms[qid])
        statistics = Statistics(query_model, background_model, para["mu"], a, b,\
            sentence_mu, top_percent, sim_threshold, doc_num, stopwords, profile, wiki_model)
        for single_dir in queries[qid]._dirs:
            ...
            if candidate_documents == None:
                continue
            candidate_sentences = select_sentence(candidate_documents, statistics, query_model,secs)
            if len(candidate_sentences) == 0:
                continue 
